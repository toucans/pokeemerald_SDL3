...
| `../docs/music-sc88-compressed.pak` | The same pak with the sample PCM as Ogg/Opus (120 kbps mono) — **the web transport format** (~16 MB vs ~50): the site player fetches it and the web game build preloads it (`../tools/sc88z-prejs.js`). `sc88z.js` rebuilds the exact M4AO bytes in the browser; distinct `M4AZ` magic so the engine can't load it by mistake; the native game reads the original. `tools/compress_sc88.py`. |
...
| `../docs/player.js` + `index.html` | Main-thread shim + tiny UI: every song has `GBA` (game version) and `SC-88` (engine GM synth) buttons, plus `fluidsynth` (offline opus render from `tools/render_compare.py`, local-only/gitignored) when present — for A/B'ing the engine against a second synth. The SC-88 pak is fetched lazily on first use — as the Opus transport pak (`?sc88full` fetches the 50 MB original instead, for A/B'ing the codec). |
