...
bool m4a_sc88_init(const char *pak_path);   /* optional second pak (SC-88) */
#endif
...
/* the SC-88 soundtrack: composers' 480-tpqn MIDIs + SC-88Pro samples */
uint32_t m4a_sc88_count(void);
const char *m4a_sc88_name(uint32_t i);
int m4a_sc88_find(const char *name);    /* -1 if that song has no SC-88 version */
bool m4a_play_sc88_index(uint32_t i);
bool m4a_sc88_is_current(void);
uint32_t m4a_sc88_nvids(uint32_t i);    /* per-song instrument slots (viz) */
