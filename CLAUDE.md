...
- **Music is done.** Both soundtracks are committed in
  ...
  `music-sc88-compressed.pak` is its Opus transport for the web — site and
  web game build; native plays the original — regenerate with
  `tools/compress_sc88.py` whenever `music-sc88.pak` changes); the game
  build has no music step. Never extract or
